...
class TestIMStat(tm.TestCase):
    ...
    def test_nilfilename(self):            
        try:
            sum = self.s.summary(table={'caslib':self.srcLib, 'name':None}) 
        except TypeError:
            # Expected: TypeError: table.name is a required argument
            pass  
        
    def test_badfilename(self):     
        sum = self.s.summary(table={'caslib':self.srcLib, 'name':'/hps/badfilename.sashdat'})
    ...
    def test_badwhere(self):     
        sum = self.s.summary( table={'caslib':self.srcLib, 'name':self.tablename, 'where':'FOOBAR<5'} )
        self.assertEqual(sum, {})
        self.assertContainsMessage(sum, "ERROR: The WHERE clause 'FOOBAR<5' could not be resolved.")
        
    def test_countwhere(self):     
        sum = self.s.numrows( table={'caslib':self.srcLib, 'name':self.tablename, 'where':'1<5'} )
    ...
    def test_summary2(self):        
        ...
        expected = [
            ['MSRP', 10280.0, 192465.0, 428.0, 0.0, 32774.85514018692,
                14027638, 19431.71667371752, 939.2674776639877, 377591612.88763154, 620985422112, 
                161231618703.01868, 59.288489882267406, 34.894059380933605, 4.160411927480192e-127],                    
            ['Invoice', 9875.0, 173560.0, 428.0, 0.0, 30014.70093457944,
                12846292, 17642.117750314756, 852.7639486634733, 311244318.715971, 518478936590, 
                132901324091.7196, 58.77825599118186, 35.19696274874321, 2.684397703921224e-128],            
            ['EngineSize', 1.3, 8.3000000000000007, 428.0, 0.0, 3.1967289719626195,
                1368.2000000000012, 1.1085947183514742, 0.053585948289683515, 1.2289822495567844, 4898.539999999997,
                524.7754205607371, 34.679033727118153, 59.6561052662767, 3.133744529671493e-209],                    
            ['Cylinders', 3.0, 12.0, 426.0, 2.0, 5.807511737089202,
                2474, 1.5584426332202244, 0.07550679229836678, 2.4287434410383866, 15400, 
                1032.2159624413143, 26.834945907510733, 76.91376577276239, 1.5155688723940396e-251],                    
            ['Horsepower', 73.0, 500.0, 428.0, 0.0, 215.88551401869159,
                92399.0, 71.836031583690726, 3.4723256480095284, 5160.415433693011, 22151103,
                2203497.390186917, 33.275058732042154, 62.17317610819295, 4.18534404473185e-216],
            ['MPG_City', 10.0, 60.0, 428.0, 0.0, 20.060747663551403,
                8586.0, 5.238217638649048, 0.25319880644223236, 27.438924029854007, 183958, 
                11716.420560747647, 26.111776721893694, 79.22923470860945, 1.8662836393934126e-257],
            ['MPG_Highway', 12.0, 66.0, 428.0, 0.0, 26.843457943925234, 
                11489.0, 5.7412007169842276, 0.27751141120218892, 32.96138567270021, 322479, 
                14074.51168224297, 21.387709172854464, 96.72920413484424, 1.6656208376181815e-292],
            ['Weight', 1850.0, 7190.0, 428.0, 0.0, 3577.9532710280373, 
                1531364.0, 758.98321460987063, 36.686838406826887, 576055.520059533, 5725124540, 
                245975707.0654211, 21.212776051482511, 97.52689047094972, 5.812546635778978e-294],
            ['Wheelbase', 89.0, 144.0, 428.0, 0.0, 108.15420560747664,
                46290.0, 8.3118129910895107, 0.40176664543050045, 69.08623519884436, 5035958,
                29499.822429906577, 7.6851500544098297, 269.19657676307946, 0.0],
            ['Length', 143.0, 238.0, 428.0, 0.0, 186.36214953271028,
                79763.0, 14.357991256895621, 0.69401970287198034, 206.1519129330911, 14952831,
                88026.86682243086, 7.7043494577076173, 268.52573314779056, 0.0]                                                            
        ]        
        ...
        # Compatibility between Py3 and Py2
        try:
            unicode
        except:
            unicode = str
    ...
    def test_regression(self):  
        r = self.s.regression( table={'caslib':self.srcLib, 'name':self.tablename, 'vars':['MPG_City','MPG_Highway']} ) 
        self.assertEqual( r.status, None ) 
